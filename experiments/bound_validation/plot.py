...
FIGURES_DIR = os.path.join(ROOT, "results", "bound_validation", "figures")
DATA_DIR = os.path.join(ROOT, "results", "bound_validation", "data")
...
def plot_alpha_sweep_combined(dist):
    ...
    out = os.path.join(FIGURES_DIR, "paper", f"alpha_sweep_{dist}.png")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    fig.savefig(out, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {out}")
...
def plot_epsilon_sweep_combined(dist, alpha):
    ...
    out = os.path.join(
        FIGURES_DIR, "paper", f"epsilon_sweep_{dist}_alpha{alpha}.png"
    )
    os.makedirs(os.path.dirname(out), exist_ok=True)
    fig.savefig(out, dpi=300, bbox_inches="tight")
    plt.close(fig)
